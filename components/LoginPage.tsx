...
const LoginPage: React.FC<LoginPageProps> = ({ onLoginSuccess, onSwitchToRegister, seasonTheme }) => {
  ...
  return (
    <div className="w-full max-w-md bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm rounded-2xl shadow-lg p-8 space-y-6 transition-colors duration-300">
      <div className="text-center">
        <h2 className="text-3xl font-bold text-gray-800 dark:text-gray-100">{t('welcome_back')}</h2>
        <p className={`${seasonTheme.textMuted}`}>{t('login_prompt')}</p>
      </div>
      ...
      <p className="text-center text-sm text-gray-600 dark:text-gray-400">
        {t('no_account')}{' '}
        <button onClick={onSwitchToRegister} className={`font-medium ${seasonTheme.text} hover:opacity-80`}>
          {t('register_here')}
        </button>
      </p>
    </div>
